...
const ADMIN_CONCEPT_GUIDES: ConceptGuide[] = [
  ...
  },
  {
    ...
    plainExplanation: 'Every automated process, background job, microservice, and integration script that needs to call a protected API is a workload identity — a non-human user that needs credentials but has no human to log in interactively. Service identities are first-class users marked as service accounts: each has a name (describing the workload), an owner (the team accountable for it), role and group memberships, a set of allowed scopes and audiences (constraining token issuance), and one or more OAuth client credential pairs with expiry and rotation history. Workloads use the issued client ID and one-time client secret at /oauth/token with grant_type=client_credentials to receive access tokens. Rather than sharing a human user credential for automation (a common anti-pattern), each workload gets its own tightly scoped user identity that can be independently rotated, revoked, permissioned, and audited.',
...
const ENTITY_FIELD_TUTORIALS: EntityFieldGuide[] = [
  ...
  },
  {
    ...
    purpose: 'Defines a non-human user used for machine-to-machine token issuance, role/group authorization, and explicit scope/audience boundaries.',
