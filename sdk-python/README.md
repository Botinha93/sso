...
- `admin.users.list(...)` — supports `group`, `active`, `includeServiceUsers`, `search`, `page`, `pageSize`, and `customAttribute.{key}` filters (see `docs/ADMIN_USER_FILTERING.md`). Excludes M2M/service identities unless `includeServiceUsers=True`.
- `admin.groups.users(group_id, ...)` — same filters for direct group members (same default exclusion of service identities)
